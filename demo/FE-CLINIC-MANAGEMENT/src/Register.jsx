  ...
  const getRegistrationErrorMessage = (error) => {
    ...
    if (Array.isArray(data?.violations) && data.violations.length > 0) {
      const violations = data.violations
        .map((violation) => violation?.message || violation?.details || violation?.field)
        .filter(Boolean);

      if (violations.length > 0) {
        return `Đăng ký thất bại: ${violations.join(' • ')}`;
      }
    }

    if (data?.message || data?.details || data?.error) {
      return `Đăng ký thất bại: ${data.message || data.details || data.error}`;
    }
    ...
    return 'Đăng ký thất bại. Vui lòng kiểm tra lại thông tin hoặc thử lại sau.';
  };
